# tutaj zrobić wersję która będzie kodowała
def code_vinegre(message,keyword):
    ...
    for i in letter_indexes:
        uncoded_list.append(alphabet.index(message[i]))
    
    while len(keyword_list) < len(letter_indexes):
        for key in keyword_indexes:
            keyword_list.append(key)
    for i in range(len(letter_indexes)):
        decoded_index = (uncoded_list[i] + len(alphabet)) - keyword_list[i]
        if decoded_index > len(alphabet) - 1:
            decoded_index = decoded_index - len(alphabet)
            coded_list.append(alphabet[decoded_index])
        else:
            coded_list.append(alphabet[decoded_index])
    decoded_letters = list(zip(letter_indexes,coded_list))
